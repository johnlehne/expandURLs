...
def main():
    """create expanded links file"""
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "-i",
        "--input",
        help="filename of input file that contains shortlinks",
        required="True",
    )
    parser.add_argument(
        "-o",
        "--output",
        help="filename for output of expanded links",
        required="True"
    )
            ...
            if SHORT_URL in line:
                ...
                for request_line in http_request.text.split("\n"):
                    if LINE_CONTAINING_URL_IN_HTML in request_line:
                        ...
                        list_for_new_file.append(
                            description_url_split[0] + " - " + expanded_url + "\n"
                        )
            else:
